...
select * from Entity_Metadata x where x.EntityTypeFarsiName like N'%اح%ام%ارگز%'
--ETC=817

select * from entity_ahkam



  select * from Send_Receivers
  where ReceiverRoleID=18559 and SendCode in (select sendcode from sends where EntityTypeCode=817)
  

  select EntityCode,* from sends where EntityTypeCode=817 and SendCode in (select sendcode from Send_Receivers where ReceiverRoleID=18559)
...
where Code in ( 
select b.Code 
from sends a,Send_Receivers b,Actions c,entity_ahkam d,Entity_Metadata e
where 
1=1
and a.EntityTypeCode=817 
...
ResponseDate=getdate(),FinishedOperation=1
where Code in ( 
select b.Code from ActiveSends a,ActiveSend_Receivers b,Actions c,entity_ahkam d,Entity_Metadata e
where 
1=1
and a.EntityTypeCode=817 
and a.SendCode=b.SendCode
