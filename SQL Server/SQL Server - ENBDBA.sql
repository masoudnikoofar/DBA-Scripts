USE MASTER;
CREATE VIEW [DBO].[ENBDBA_SESSION]
AS
...
CREATE VIEW [DBO].[ENBDBA_JOB_RUN_DETAILES]
AS
...
CREATE VIEW [DBO].[ENBDBA_JOB_RUN_DETAILES_DAILY]
AS
SELECT     A.NAME AS JOB_NAME, B.RUN_STATUS, 
                      CASE B.RUN_STATUS WHEN 0 THEN 'FAILED' WHEN 1 THEN 'SUCCESSFUL' WHEN 2 THEN 'RETRY' WHEN 3 THEN 'CANCELLED' ELSE 'OTHER' END AS RUN_STATUS_DESC, 
                      B.MESSAGE, B.RUN_DATE, B.RUN_TIME, MSDB.DBO.AGENT_DATETIME(B.RUN_DATE, B.RUN_TIME) AS RUN_DATETIME, B.RUN_DURATION
...
use master
grant select on [master].[dbo].[ENBDBA_JOB_RUN_DETAILES_DAILY] to DBMONITORING
use msdb
...
select * From [DBO].[ENBDBA_BACKUP_RESTORE_PROGRESS]
CREATE VIEW [DBO].[ENBDBA_BACKUP_RESTORE_PROGRESS] as
